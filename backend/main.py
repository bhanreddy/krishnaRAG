...
try:
    engine = RAGEngine(
        corpus_path="data/corpus/geetha_verses.txt",
...
async def search(req: QueryRequest):
    ...
    try:
        docs = engine.search(req.question, top_k=req.top_k)
        return {
...
async def query(req: QueryRequest):
    ...
    # Check for predefined answers (works without Gemini!)
    has_predefined, predefined_answer = get_predefined_answer(req.question)
    if has_predefined:
        return QueryResponse(
            question=req.question,
    ...
    if not engine:
        return QueryResponse(
            question=req.question,
            answer="I apologize, but the RAG engine is not initialized. Please try asking about yoga, dharma, karma, moksha, or other Bhagavad Gita concepts which have predefined answers.",
            retrieved=[],
            passage_count=0
        )
    ...
    try:
        ...
        prompt = f"""You are a knowledgeable guide to the Bhagavad Gita. 
Answer the following question accurately using the provided passages.
Be thoughtful and respectful of the sacred teachings.
...
async def lifespan(app: FastAPI):
    ...
    global engine  # Ensure engine is accessible if you need it at shutdown
    try:
        client = get_gemini_client(api_key=GEMINI_API_KEY)
        if client.is_available():
            logger.info(f"✓ Gemini API Connected: {client.model_name}")
            logger.info(f"  Available models: {client.list_models()}") # <-- Better log
        else:
    ...
    yield  # --- Your application runs here ---
